...
BGC = {
    ...
    draw: function() {
        ...
        for (var i = 0; i < numOfPoints; i++) {
            var pt = BGC.points[i];
            pt.vx += (Math.random() - .5) / 12;
            pt.vy += (Math.random() - .5) / 12;
            pt.vx = clampNum(-.2, pt.vx, .2);
            pt.vy = clampNum(-.2, pt.vy, .2);
            pt.x += pt.vx;
            pt.y += pt.vy;
        ...
        for (var i = 0; i < numOfPoints; i++) {
            ...
            for (var ii = i + 1; ii < numOfPoints; ii++) {
                ...
                if (d < window.innerWidth / cF) {
                    ...
                    var alpha = (20 * ((window.innerWidth / cF) - d) / (window.innerWidth / cF))*.5;
